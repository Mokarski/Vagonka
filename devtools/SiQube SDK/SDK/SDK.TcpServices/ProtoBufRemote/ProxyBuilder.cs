...
namespace SDK.NetworksServices.ProtoBufRemote
{
    internal class ProxyBuilder
    {
        ...
        private readonly MethodInfo mBlockingCallHelperMethodInfo;
        private readonly MethodInfo mCallWithoutResultHelperMethodInfo;
        private readonly MethodInfo mBeginAsyncCallHelperMethodInfo;
        private readonly MethodInfo mEndAsyncCallHelperMethodInfo;
        ...
        {           
            ...
            mModuleBuilder = assemblyBuilder.DefineDynamicModule(assemblyName.Name);

            mBlockingCallHelperMethodInfo = typeof(Proxy).GetMethod("BlockingCallHelper",
                BindingFlags.Instance|BindingFlags.NonPublic, null,
                new[] { typeof(string), typeof(RpcMessage.Parameter[]) }, null);
            mCallWithoutResultHelperMethodInfo = typeof(Proxy).GetMethod("CallWithoutResultHelper",
                BindingFlags.Instance|BindingFlags.NonPublic, null,
                new[] { typeof(string), typeof(RpcMessage.Parameter[]) }, null);
            mBeginAsyncCallHelperMethodInfo = typeof(Proxy).GetMethod("BeginAsyncCallHelper",
                BindingFlags.Instance | BindingFlags.NonPublic, null,
                new[] { typeof(string), typeof(RpcMessage.Parameter[]), typeof(AsyncCallback), typeof(object) }, null);
            mEndAsyncCallHelperMethodInfo = typeof(Proxy).GetMethod("EndAsyncCallHelper",
                BindingFlags.Instance | BindingFlags.NonPublic, null,
        ...
        public Type Build(Type serviceType)
        {
            if (!serviceType.IsInterface)
                throw new ArgumentException(String.Format("Type '{0}' being used to create a proxy must be an interface", serviceType.FullName));

            var service = new RpcServiceDescriptor(serviceType);

            TypeBuilder tb = mModuleBuilder.DefineType(serviceType.Name + "Proxy", TypeAttributes.Public,
            ...
            foreach (var method in service.Methods)
            {
                ...
                {
                    methodIlGen.Emit(OpCodes.Call, mCallWithoutResultHelperMethodInfo);
                }
                else
                {
                    methodIlGen.Emit(OpCodes.Call, mBlockingCallHelperMethodInfo);
                ...
                if (method.HasAsyncDeclarations)
                {
                    ...
                    beginIlGen.Emit(OpCodes.Call, mBeginAsyncCallHelperMethodInfo);
                    ...
                    endIlGen.Emit(OpCodes.Call, mEndAsyncCallHelperMethodInfo);
                    if (method.ReturnType == typeof(void))
