...
namespace SDK.UI.Style.WXGA.Sparc
{
    ...
    public class RadialInfoIcon : Widget, IInfoIcon
    {
            ...
            : base(parent, 150, 150)
        {
            ...
            mPhysical = physical;
            Init(x, y);
            mImageBg.Load("wxga.infoicon.bg.raw");
            mImageFg.Load("wxga.infoicon.fg.raw");
        }
        ...
        public override void Draw()
        {
            ...
            */
...

            {
                ...
                VG.vgSetf(VGParamType.VG_STROKE_LINE_WIDTH, 12.0f);
                VG.vgSetPaint(mPaint, VGPaintMode.VG_STROKE_PATH);
                VG.vgSetfv(VGParamType.VG_STROKE_DASH_PATTERN, 0, null);

                const float kFullScale = -256;
                const int kRadius = 110;
                VguArc(mCircle, X + Width / 2, Y + 80, kRadius, kRadius, 218, kFullScale * (Percent / 100.0f));
            }
    ...
    public class LinearInfoIcon : Widget, IInfoIcon
    {
        ...
        private void Init(int x, int y)
        {
            ...
            Format = "{0:0}";
            mActiveArea = new TextArea(this, Width - 65, (int)(Height * 0.45f), 64, 30);//
            SetValue(0);

            mActiveArea.SetAlign(Align.Left, new GfxPoint(0, 5));
            mActiveArea.SetFont(Palette.White, 26);

            AddVGPath(new VGPath(mPath, null, new VGSolidColor(new Color(0x2c2d2eff))));

            {
                var i = VGPath.OpenVGPath();
                VGU.vguRect(i, 0, 0, 60, 95);
        ...
        public override void Draw()
        {
...

            {
                ...
                VG.vgSetf(VGParamType.VG_STROKE_LINE_WIDTH, 55.0f);
                VG.vgSetPaint(mPaint, VGPaintMode.VG_STROKE_PATH);
                VG.vgSetfv(VGParamType.VG_STROKE_DASH_PATTERN, mDashPattern.Length, mDashPattern);


                const float kFullScale = 90;
                var rv = kFullScale * (Percent / 100.0f);
                VGU.vguLine(mLinear, X + 45, Y + 45, X + 45, Y + 45 + rv);
    ...
    public class SpeedIcon : Widget
    {
        ...
        private void Init(int x)
        {
            ...
            {
                mText.Add(Text.Label(this, string.Format("{0}", i + 1), 30, -1, (i * (Width + 5)), Width, Width));
            }
    ...
    public class VagonIcon : Widget
    {
            ...
            : base(parent)
        {
            ...
            VGU.vguRect(mRects[0].Path, pos.X + 310, pos.Y + mImage.Height - 75, 65, 65);

            VGU.vguRect(mRects[1].Path, pos.X + 155, pos.Y + mImage.Height - 75, 65, 65);
            VGU.vguRect(mRects[2].Path, pos.X + 400, pos.Y + mImage.Height - 75, 65, 65);
            VGU.vguRect(mRects[3].Path, pos.X + 155, pos.Y + 10, 65, 65);
            VGU.vguRect(mRects[4].Path, pos.X + 400, pos.Y + 10, 65, 65);
    ...
    public class AngleInfoIcon : Widget
    {
        ...
        private void Init(int x, int y)
        {
            ...
            mImage.Load("./wxga.roll_tangage.raw");

            Text.Label(this, "Тангаж", 16, 0, 0, Width, 16);
            Text.Label(this, "Крен", 16, 0, Height - 20, Width, 16);

            mPaint = VG.vgCreatePaint();
            VG.vgSetParameterfv(mPaint, (int)VGPaintParamType.VG_PAINT_COLOR, 4, (new Color(0xEDAB18ff)).Value);

            mPitchArea = new TextArea(this, Width - 45, 120, 35, 30) { Text = "" };
            mPitchArea.SetAlign(Align.Right, new GfxPoint(0, 5));
            mPitchArea.SetFont(Palette.White, 18);
            ...
            mRollArea = new TextArea(this, Width - 45, 5, 35, 30) { Text = "" };
            mRollArea.SetAlign(Align.Right, new GfxPoint(0, 5));
            mRollArea.SetFont(Palette.White, 18);
    ...
    public class FilterInfoIcon : Widget
    {
        ...
        private void Init(int x, int y)
        {
            ...
            mImage = new Image(this, 10, 50);
            mImage.Load("./wxga.filter.raw");

            Text.Label(this, "Фильтр\\nв системе смазки", 16, 0, 0, Width, 16);

            mPaint = VG.vgCreatePaint();
            VG.vgSetParameterfv(mPaint, (int)VGPaintParamType.VG_PAINT_COLOR, 4, (new Color(0xEDAB18ff)).Value);


            mRollArea = new TextArea(this, Width - 80, 65, 78, 30) { Text = "" };
    ...
    public class VoltageInfoIcon : Widget
    {
        ...
        private void Init(int x, int y)
        {
            ...
            Text.Label(this, "Напряжение\\nв сети", 16, 0, 0, Width, 16);

            mPaint = VG.vgCreatePaint();
            VG.vgSetParameterfv(mPaint, (int)VGPaintParamType.VG_PAINT_COLOR, 4, (new Color(0xEDAB18ff)).Value);


            mRollArea = new TextArea(this, 0, 60, Width, 30) { Text = "" };
            mRollArea.SetAlign(Align.Center, new GfxPoint(0, 5));
            mRollArea.SetFont(Palette.White, 28);
            mRollArea.Text = string.Format("{0} В", mSensor.Value);

            mSensor.OnUpdate += sensor =>
                                    {
                                        mRollArea.Text = string.Format("{0} В", sensor.Value);
                                    };
    ...
    public class InvertorInfoIcon : Widget
    {
        ...
        private void Init(int x, int y)
        {
            ...
            const int kBiasX = 130;
            const int kBiasX0 = 200;
            Text.Label(this, "T, ⁰C", 24, kBiasX0, kBiasY * 4, kBiasX, 24);
            Text.Label(this, "I, A", 24, kBiasX0 + kBiasX, kBiasY * 4, kBiasX, 24);
            Text.Label(this, "U, B", 24, kBiasX0 + 2 * kBiasX, kBiasY * 4, kBiasX, 24);
            Text.Label(this, "F, Гц", 24, kBiasX0 + 3 * kBiasX, kBiasY * 4, kBiasX, 24);
            Text.Label(this, "об/мин", 24, kBiasX0 + 4 * kBiasX, kBiasY * 4, kBiasX, 24);

            //Text.Label(this, "ПЧН #1\\nПЧН #2\\nПЧН #3\\nПЧН #4", 18, 15, 22, kBiasX0 - 15, kBiasY, Align.Left);

            mTable = new Table(this);
            mTableLine = new TableLine(kBiasY / 2);
            mTableLine.AddColumn(kBiasX0 - 15, null, Align.Center, 24, Palette.White, new GfxPoint(0, 8));
            //mTableLine.Text[0].Text = "ПЧН #4";
            mTableLine.AddColumn(kBiasX, null, Align.Center, 24, Palette.White, new GfxPoint(0, 8));
            mTableLine.AddColumn(kBiasX, null, Align.Center, 24, Palette.White, new GfxPoint(0, 8));
            mTableLine.AddColumn(kBiasX, null, Align.Center, 24, Palette.White, new GfxPoint(0, 8));
            mTableLine.AddColumn(kBiasX, null, Align.Center, 24, Palette.White, new GfxPoint(0, 8));
            mTableLine.AddColumn(kBiasX, null, Align.Center, 24, Palette.White, new GfxPoint(0, 8));
            ...
            mTableStatusLine.AddColumn(kBiasX0 - 15, null, Align.Center, 24, new Color(0xD1D3D4FF), new GfxPoint(0, 8));
            mTableStatusLine.Text[0].Text = "статус:";
            mTableStatusLine.AddColumn(kBiasX * 5, null, Align.Center, 24, Palette.White, new GfxPoint(0, 8));
