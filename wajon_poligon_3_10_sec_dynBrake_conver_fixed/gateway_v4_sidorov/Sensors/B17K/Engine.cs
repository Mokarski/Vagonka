...
namespace Sensors.B17K
{
    ...
    internal static class Engine
    {
        ...
        private static int enelVOLTAGE = 500; // уровень напряжения для разрешения работы
        /// <summary>
        ...
        private static readonly DelayedTask MotionSpeedStop = new DelayedTask(3000); // опускаем скорость до минимальной, если нет нажатия на педаль    
        ...
        private static bool IsCharged()
        {
            ...
            return Voltages.Count(s => s.ValueAsInt < enelVOLTAGE) <= 0;
        }
            ...
            try
            {
                ...
                switch (State)
                {
                    ...
                    case SystemState.Failure:
                        {
                            string fn = "";
                            if (fisrtstart == true) mJournal.Error("Режим защиты. Сброс и запуск системы.", MessageLevel.System);
        ...
        private static void Stop() //Новый стоп с динамическим торможением
        {
            // выставляем скорость в 2 Гц перед тем, как полностью остановить - для преотвращения выбега
            // выставляем скорость в 1 Гц(1.00=100) перед тем, как полностью остановить - для преотвращения выбега
            //mJournal.Debug("uz.status== ", MessageLevel.System);
            mJournal.Debug(string.Format("Установлен режим: UZ4.status{0} Uz4.Hz{1} ", mUz4.Status, mUz4.GetFrequency()), MessageLevel.System);
            if (mUz4.GetFrequency() < 200 && mUz4.Status == EngineStatus.Activated)
            {
                mJournal.Debug("uz.status== activated and freq  < 200 ", MessageLevel.System);
                if (!MotionSpeedStop.IsStart)
                {
                    mJournal.Debug("Запуск таймера 9 сек ", MessageLevel.System);
                    mUz2.Frequency(0);
                    mUz3.Frequency(0);
                    mUz4.Frequency(0);
                    mUz5.Frequency(0);
                    MotionSpeedStop.Start();
                }
                MotionSpeedStop.OnTimeout += sender =>
                {
                    mJournal.Debug("->9сек истекло:", MessageLevel.System);
                    mUz2.Off(true);
            ...
            else
            {
                if (Mode == EngineMode.Motion)
                {
                    if (Invertors.Count(s => s.GetFrequency() <= 100) == Invertors.Count) //проверка что частота стала меньше 1гц на всех четырех ПЧН
                    {
                        /*
                        if (!MotionSpeedStop.IsStart)
                        {
                         mJournal.Debug("Установка частоты в Ноль по таймеру 9сек ", MessageLevel.System);
                            MotionSpeedStop.Start(); //начинаем отсчет 9 сек  полного стопа
                            mUz2.Frequency(0); //понижаем скорость еще и оснаваливаем подаем 0Гц на ПЧН
                            mUz3.Frequency(0);
                            mUz4.Frequency(0);
                            mUz5.Frequency(0);
                    ...
                    if (Invertors.Count(s => s.GetFrequency() == 100) == Invertors.Count) //проверка что частота стала 0гц на всех четырех ПЧН
                    {
                        /*
                        MotionSpeedStop.OnTimeout += sender =>
                        {
                         ...
                         */
                        mJournal.Debug("Uz2-off direction =null ", MessageLevel.System);
                        mUz2.Off(true);
                        mLastDirection = null; // выставляем неопределенное состояние, обозначающее, что у нас была остановка                    
                    }
        ...
        {            
            ...
            else
            {
                if (Mode == EngineMode.Motion)
                {
                    ...
                    if (Invertors.Count(s => s.GetFrequency() == 0) == Invertors.Count) //проверка что частота стала 0гц на всех четырех ПЧН
                    {

                        ResetMotionSpeedToMin.OnTimeout += sender =>
                        {
                            mJournal.Debug("(9сек истекло 1", MessageLevel.System);
                            mUz2.Off(true);    
                        };
                        //mUz2.Off(true);
                        mLastDirection = null; // выставляем неопределенное состояние, обозначающее, что у нас была остановка                    
