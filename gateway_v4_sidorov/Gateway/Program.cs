...
namespace Gateway
{
    class Program
    {
        ...
        volatile static bool foilstationstate = false; // статус маслостанции, для подсчета маслочасов и моточасов

        static bool checkValueAsBool(float f)
        {
            if (float.IsNaN(f)) return false;
            if (f == 0.0f) return false;
            if (((Int32)f) > 0) return true;
            return false;
        }
