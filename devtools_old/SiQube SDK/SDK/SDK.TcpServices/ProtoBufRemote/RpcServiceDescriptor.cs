...
namespace SDK.NetworksServices.ProtoBufRemote
{
    ...
    internal class RpcServiceDescriptor
    {
        ...
        private readonly IDictionary<string, RpcMethodDescriptor> mMethods =
        ...
        /// </summary>
        public IEnumerable<RpcMethodDescriptor> Methods { get { return mMethods.Values; } }
        ...
        public RpcServiceDescriptor(Type serviceType)
        {
            ...
            else
            {
                if (!typeWithServiceAttr.IsInterface && !serviceAttr.IsAttributedMethodsOnly)
                    throw new ArgumentException(
                        "Only interface types can be used as services without explicit attributes");
                mName = serviceAttr.Name ?? typeWithServiceAttr.Name;
        ...
        public RpcMethodDescriptor GetMethod(string methodName)
        {
            ...
            if (mMethods.TryGetValue(methodName, out method))
                return method;
            return null;
        }
